package com.jh.discord.dailyFortune.client;
...
public class FortuneCrawler {
		...
		try {
			for (String name : NAMES) {
				...
				String url = "https://search.naver.com/search.naver?query="
						+ URLEncoder.encode(name + " 운세", StandardCharsets.UTF_8);
				Document doc = Jsoup.connect(url).referrer("https://www.naver.com").userAgent(USER_AGENT).timeout(5000)
						.get();
	...
	public String detailFortune(String name) {
		...
		try {
			...
			if (todayPanel == null)
				return searchName + "의 정보를 찾을 수 없습니다.";
